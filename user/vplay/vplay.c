...
#if 1 // 05-05-2010, Jared, for swapping bytes
// 0: enable stereo recording; 1: disable stereo recording
#define DISABLE_STEREO_RECORDING 0  
#define swab16(x) \
	((unsigned short)( \
		(((unsigned short)(x) & (unsigned short)0x00ffU) << 8) | \
		(((unsigned short)(x) & (unsigned short)0xff00U) >> 8) ))

#define swab32(x) ( x<<24 | x>>24 | (x & (unsigned int)0x0000ff00UL)<<8 | (x & (unsigned int)0x00ff0000UL)>>8 )
...
int verbose_mode = 0;
int convert      = 0;
#if 0 // 05-05-2010, Jared, swap the byte orders
int record_type  = VOC_FMT;
#else
int record_type  = WAVE_FMT;
#endif
u_long count;
...
int main (int argc, char *argv[])
{
  ...
  while ((c = getopt (argc, argv, "aqs:St:b:vrwd")) != EOF)
    switch (c) {
      case 'S':
#if DISABLE_STEREO_RECORDING // 05-05-2010, Jared, for swapping bytes
	if ( direction == RECORD ) {
	fprintf (stderr, "Sorry! We don't support to record in stereo mode!\nPlease, try the Mono mode.\n");
		exit (-1);
	}
#endif
...
#else
	fprintf (stderr, 
	 "Usage: %s [-qwraS] [-t secs] [-s Hz] [-b 8|12|16] [filename]\n", command);
#endif
	exit (-1);
    }
...
// 05-05-2010, Jared, swap the byte orders
//printf("wp->main_chunk:%lx, swap:%lx\n", wp->main_chunk, swab32(wp->main_chunk));
//printf("wp->chunk_type:%lx, swap:%lx\n", wp->chunk_type, swab32(wp->chunk_type));
//printf("wp->sub_chunk:%lx, swap:%lx\n", wp->sub_chunk, swab32(wp->sub_chunk));
  if (wp->main_chunk == swab32(RIFF) && wp->chunk_type == swab32(WAVE) &&
      wp->sub_chunk == swab32(FMT) /*&& wp->data_chunk == swab32(DATA)*/) {

    if (wp->format != swab16(PCM_CODE)) {
      fprintf (stderr, "%s: can't play not PCM-coded WAVE-files\n", command);
      exit (-1);
    }
    if (swab16(wp->modus) > 2) {
      fprintf (stderr, "%s: can't play WAVE-files with %d tracks\n",
               command, swab16(wp->modus));
      exit (-1);
    }
    dsp_stereo = (wp->modus == swab16(WAVE_STEREO)) ? 1 : 0;
    samplesize = swab16(wp->bit_p_spl);
    dsp_speed = swab32(wp->sample_fq); 
    count = swab32(wp->data_length);
    return 0;
  }
  return -1;
}
...
void start_wave(int fd, u_long cnt)
{
  ...
  wh.main_chunk = swab32(RIFF);
  wh.length     = swab32( (cnt + sizeof(WaveHeader) - 8) ); 
  wh.chunk_type = swab32(WAVE);
  wh.sub_chunk  = swab32(FMT);
  wh.sc_len     = swab32(16);
  wh.format     = swab16(PCM_CODE);
  wh.modus      = swab16(dsp_stereo ? 2 : 1);
  
  wh.sample_fq  = swab32(dsp_speed); 

  
  wh.byte_p_spl = swab16( ((samplesize == 8) ? 1 : 2) * (dsp_stereo ? 2 : 1) );
#if 0 // 05-05-2010, Jared
  wh.byte_p_sec = swab32( dsp_speed * wh.modus * wh.byte_p_spl );
#else
  wh.byte_p_sec = swab32( dsp_speed * (dsp_stereo ? 2 : 1) * \
	( ((samplesize == 8) ? 1 : 2) * (dsp_stereo ? 2 : 1) ) );
#endif
  wh.bit_p_spl  = swab16(samplesize);
  wh.data_chunk = swab32(DATA);
  wh.data_length= swab32(cnt);
...
void recplay (int fd, int loaded, u_long count, int rtype, char *name)
{
	...
	if (tmps != samplesize)
	{
		...
		printf (stderr, "%s: unable to set %d bit sample size",command, samplesize);
	    if (samplesize == 16) 
		{
			samplesize = 8;
			ioctl(audio, SNDCTL_DSP_SAMPLESIZE, &samplesize);
...
//printf("%s[%d]convert:%d, samplesize:%d\n",__FUNCTION__,__LINE__,convert, samplesize);
	/* change byte order if necessary */
	if(convert && (samplesize==16)){
	  long i;

	  for(i=0; i< real_l; i+=2)
	    *( (short*) (audiobuf+i) ) = htons( *( (short*) (audiobuf+i)));
	}
...
{
	int fd, ofs;  
	set_dsp_speed (&dsp_speed);//MONK: add this function to let driver reset the dsp_speed first,bloodly important
	if (direction == PLAY)
	{
