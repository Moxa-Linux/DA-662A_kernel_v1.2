...
#define __CONSOLE_WITHOUT_LOGIN__ // Victor, following add by Victor Yu. 11-30-2004
...
#ifdef __CONSOLE_WITHOUT_LOGIN__   // add by Victor Yu. 08-26-2005, to support Ctrl-C
		if ( strcmp(ch->id, "con") == 0 ) {
...
#endif
  // following add by Victor Yu. 11-30-2004
#ifdef __CONSOLE_WITHOUT_LOGIN__
  if ( strcmp(console_dev, "/dev/null") ) {
        for(old = newFamily; old; old = old->next) {
                if(strcmp(old->id, "con") == 0 ) {
                        break;
                }
        }
        if (old) goto skip_console;
        ...
        ch->action = RESPAWN;
        strncpy(ch->id, "con", sizeof(utproto.ut_id) + 1); /* Hack for different libs. */
        strncpy(ch->process, "/bin/sh --login", sizeof(ch->process) - 1);
