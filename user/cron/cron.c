 ...
 */
#if 0	// mask by Victor Yu. 03-07-2006
#ifndef CRONFILE
#define CRONFILE	"/etc/config/crontab"
#endif
#else	// add by Victor Yu. 03-07-2006
#undef CRONFILE
#define CRONFILE	"/etc/cron.d/crontab"
#endif
...
static void check_runs(struct tm *now)
{
	...
	char     s[26];
	char	*q;
	int status; //Wade: add this for fix vfork problem
	...
	for (task = tasklist; task != NULL; task = task->next) {
		if (bit_test(task->minutes, now->tm_min) &&
			...
			/* test */
			//printf("parent wait status\n");
			//Wade: add this for fix vfork problem
			wait(&status);
...
int main(int argc, char *argv[])
{
	...
	//Wade: comment this for fixing vfork problem(cause /etc
	//init.d/networkin can't work for dhcp
