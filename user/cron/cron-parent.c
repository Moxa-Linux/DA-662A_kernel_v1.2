...
#include <string.h>
#include <syslog.h>
#if 1 // Jared, 2010-12-15, Fix for libc.so.6 section .tbss mismatches non-TLS reference
#include <errno.h>
#endif
...
static inline void setupfds(void)
{
	...
	if (mailuser == NULL || mailhost == NULL) {
		if (outfile != NULL) {
			if(!attech_file){
				unlink(outfile);
				i = open(outfile, O_WRONLY|O_CREAT|O_TRUNC, 0644);			}else i = open(outfile, O_WRONLY|O_CREAT|O_APPEND, 0644);
		}
		if (outfile == NULL || i == -1)
...
{
	int pid, p2;
#if 0 // Jared, 2010-12-15, Fix for libc.so.6 section .tbss mismatches non-TLS reference
	extern int errno;
#endif
#ifdef ERRFD
	dup2(2, ERRFD);
