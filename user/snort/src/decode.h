...
/* $Id: decode.h,v 1.72 2003/04/28 13:08:07 chrisgreen Exp $ */
...
#define ETHERNET_MAX_LEN_ENCAP          1500  /* 802.3 (+LLC) or ether II ? */
...
{
    u_int8_t code;
    u_int32_t len; /* length of the data section */
    u_int8_t *data;
}        Options;
...
#define URI_LENGTH       512  /* Max length of a single HTTP URI */
#define URI_VALUE_LENGTH 512 /* Max length of a single HTTP query param value */
#define URI_COUNT        5   /* Max Number of HTTP/1.1 requests in a single segment */
#define URI_PARAM_COUNT  50 /* Max number of HTTP parameters */
#define URI_PARAM_LEN    255 /* Max size of a decoded URI parameter */

#define URI_FOUND_HEX            0x0001  /* %20 */
#define URI_FOUND_UNICODE        0x0002  /* %c0%cf */
#define URI_FOUND_UNICODE_IIS    0x0004  /* %uc0af */
#define URI_HTTP_1_0             0x0010  /* %uc0af */
#define URI_HTTP_1_1             0x0020
#define URI_HTTP_UNKNOWN         0x0040
#define URI_DECODED              0x1000  /* this one has been done */

typedef struct _UriParam
{
    u_int8_t *key; /* ?q = */
    u_int32_t keylen;
    u_int8_t *raw_value; /* the raw bytes to be decoded later */
    u_int32_t rawlen;
    u_int8_t decoded[URI_PARAM_LEN]; /* decoded bytes */
    u_int32_t decode_flags; 
} UriParam;

typedef struct _HttpUri
{
    u_int8_t uri[URI_LENGTH + 1];  /* static buffer for uri length */
    u_int16_t length;
    UriParam params[URI_PARAM_COUNT];
