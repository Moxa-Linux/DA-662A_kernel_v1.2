/*
**  $Id: mwm.c,v 1.7 2003/04/21 15:14:18 chrisgreen Exp $
**
...
**
** A pattern match tracking mechanism 
** has been added to reduce the effect of DOS attacks, and generally improve the
** worst case performanmce scenario.
**
** 
...
int mwmAddPattern( void * pv, unsigned char * P, int m, unsigned id )
{
    ...
    return mwmAddPatternEx( pv, P, m, 0, id, 0, 64*1024, 0 );
}
...
static void mwmPrepHashedPatternGroups(MWM_STRUCT * ps)
{
  ...
  */
  for(i=0;i<ps->msNumPatterns;i++)
  {
    ps->msPatArray[i].psIID = i; /* internal id */
    ...
    else if( ps->msPatArray[i].psLen == 1 )
    {
       ...
       ningroup = 1;  
       while( (++i < ps->msNumPatterns) && (hindex == ps->msPatArray[i].psPat[0]) )
         ningroup++;
       ps->msNumArray1[ sindex ] = ningroup;
       i--;
    }
  }
...

{
  ...
  for( T = Tx, B = Tx + ps->msShiftLen - 1; B < Tend; T++, B++, Tleft-- )
  {
     ...
     if( (index = ps->msHash1[*T]) != (HASH_TYPE)-1 )
     {
         ...
         patrnEnd = patrn + ps->msNumArray1[index];
...
int mwmSearchExBC( MWM_STRUCT *ps, 
                   ...
                   void * data )
{
  int                 Tleft, index, nfound, k, tshift;
  unsigned char      *T, *Tend, *B;
  MWM_PATTERN_STRUCT *patrn, *patrnEnd;
  ...
  for( T = Tx, B = Tx + ps->msShiftLen - 1; B < Tend; T++, B++, Tleft-- )
  {
     ...
     for( ;patrn<patrnEnd; patrn++ )  
     {
	  ...
	  if( boIsBitSet( ps->RuleMask, patrn->psIID ) )
       ...
       /* Compare strings backward, unrolling does not help. */
       while( k >= 0 && (q[k] == p[k]) ) k--;
...
*/
static int match (  unsigned id, int index, void * data )
{
   static int x=1;
   
   printf(" pattern matched: index= %d, id=%s, \n", 
            index, (char*)id,
            );
...
int CDECL main ( int argc, char ** argv )
{
   ...
   int            nmatches, i, bm = 0;
   MWM_STRUCT    *ps;
   int            npats, len, stat, noCase=0;

   if( argc < 3 )
   {
      printf("usage: %s Text Pat1 ... Patn [-nocase] \n",argv[0]);
      exit(1);
   }

   if( strcmp(argv[argc-1],"-nocase")==0 ) noCase = 1;
   ...
   {
      len = strlen( argv[i] );
      mwmAddPatternEx( ps, (unsigned char*)argv[i], len, 
                       noCase, 0, 0, 
                      (unsigned)argv[i],  3000 );
   }
   ...
   stat = mwmSearch( (unsigned char*)argv[1], strlen(argv[1]), match, 0 ); 

   if( stat == 0 )
   {
     printf("%s has no pattern matches\n",argv[1]);
   }else{
     printf("%s has %d pattern matches in list\n",argv[1], stat);
   }
