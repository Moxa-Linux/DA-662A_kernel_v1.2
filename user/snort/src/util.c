/* $Id: util.c,v 1.29 2003/03/14 08:28:51 chris_reid Exp $ */
/*
...
static void SigChrootHupHandler(int signal);
...
void SysLog(int priority, const char *format, ...)
{
    ...
    openlog("snort", LOG_PID, LOG_DAEMON);
...
void CreateApplicationEventLogEntry(const char *msg)
{
    ...
    /* prepare to write to Application log on local host
      ...
      */
    hEventLog = RegisterEventSource(NULL, "SnortService");
    if (hEventLog == NULL)
    {
...
void DropStats(int iParamIgnored)
{
    ...
    printf("    Management Packets: %-10ld (%.3f%%)\n", 
           pc.wifi_mgmt, CalcPct((float) pc.wifi_mgmt
                     , recv + drop));
    printf("    Control Packets:    %-10ld (%.3f%%)\n", 
           pc.wifi_control, CalcPct((float) pc.wifi_control
                        , recv + drop));
    printf("    Data Packets:       %-10ld (%.3f%%)\n", 
...
void SetChroot(char *directory, char **logstore)
{
    ...
    signal(SIGHUP, SigChrootHupHandler);
#endif /* !WIN32 */
}
...
/** 
 * dummy signal handler for chroot
 * 
 * @param signal signal to exec
 */
static void SigChrootHupHandler(int signal)
{
    LogMessage("Signal %d does not work under chroot\n", signal);
}
