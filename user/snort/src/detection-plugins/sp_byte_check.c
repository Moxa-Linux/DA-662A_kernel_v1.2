/* $Id: sp_byte_check.c,v 1.19 2003/05/31 13:34:38 cazz Exp $ */
...
void ByteTestParse(char *data, ByteTestData *idx, OptTreeNode *otn)
{
    ...
    switch(*cptr)
    {
        ...
        case '!': idx->operator = BT_NOT_EQUAL;
                  break;
    ...
    {
        FatalError("%s(%d): string, dec and oct modifiers must be used in conjunction \n"
...
int ByteTest(Packet *p, struct _OptTreeNode *otn, OptFpList *fp_list)
{
    ...
    if(doe_ptr)
    {
        ...
        if(!inBounds(start_ptr, end_ptr, doe_ptr))
        {
            ...
            /* just incase something uses this function indirectly */
            doe_ptr = NULL;  
            return 0;
        }
    }
    ...
    switch(btd->operator)
    {
        ...
        case BT_NOT_EQUAL: if(value != btd->cmp_value)
                               success = 1;
                           break;
