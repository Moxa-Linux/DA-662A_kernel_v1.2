/* $Id: snort.c,v 1.193 2003/05/28 15:50:16 chris_reid Exp $ */
/*
...
int SnortMain(int argc, char *argv[])
{
    ...
    /* make sure we don't close important fd's in daemon mode */
...
int ShowUsage(char *progname)
{
    ...
    FPUTS_BOTH ("        -M <wrkst> Sends SMB message to workstations in file <wrkst>\n");
    FPUTS_BOTH ("                   (Requires smbclient to be in PATH)\n");
#endif
...
int ParseCmdLine(int argc, char *argv[])
{
    ...
    valid_options = "?A:bB:c:CdDefF:g:h:i:Ik:l:L:m:M:n:NoOpP:qr:R:sS:t:Tu:UvVwXyz";
#else
    /* Win32 does not support:  -D, -g, -m, -t, -u */
    /* Win32 no longer supports an argument to -s, either! */
    valid_options = "?A:bB:c:CdeEfF:h:i:Ik:l:L:M:n:NoOpP:qr:R:sS:TUvVwWXyz";
#endif
...
#ifdef ENABLE_SMB_ALERTS
                pv.alert_mode = ALERT_SMB;
                strlcpy(pv.smbmsg_dir, optarg, STD_BUF);
                pv.alert_cmd_override = 1;
#else
                FatalError("SMB support not compiled into program, exiting...\n");
#endif
                break;
