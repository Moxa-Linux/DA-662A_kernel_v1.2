/* $Id: spp_frag2.c,v 1.61 2003/05/13 15:55:33 chrisgreen Exp $ */
/*
    ...
    if(ft == NULL)
    {
        ...
        if(f2data.state_protection)
        {
            if(f2_emergency.new_frag_count >= f2data.suspend_threshold)
            {
                ...
                {
                    SetEvent(&event, GENERATOR_SPP_FRAG2, 
                            FRAG2_SUSPEND, 1, 0, 5, 0);
                    CallAlertFuncs(NULL, FRAG2_SUSPEND_STR, NULL, &event);
                    CallLogFuncs(p,  FRAG2_SUSPEND_STR, NULL, &event);
            ...
            else if (f2_emergency.new_frag_count >= f2data.sp_threshold)
            {
                ...
                {
                    SetEvent(&event, GENERATOR_SPP_FRAG2, 
                            FRAG2_EMERGENCY, 1, 0, 5, 0);
                    CallAlertFuncs(p, FRAG2_EMERGENCY_STR, NULL, &event);
                    CallLogFuncs(p,  FRAG2_EMERGENCY_STR, NULL, &event);
                    alert_once_emerg  = 1;
                }
            }
        }
...
int InsertFrag(Packet *p, FragTracker *ft)
{
    ...
    if(ft->frag_bytes + p->dsize > 70000)
    {
        ...
        SetEvent(&event, GENERATOR_SPP_FRAG2, FRAG2_OVERSIZE_FRAG, 1, 0, 5, 0);
...
void RebuildFrag(FragTracker *ft, Packet *p)
{
    ...
    defrag_pkt->iph->ip_len = htons(defrag_pkt->pkth->len - ETHERNET_HEADER_LEN);
