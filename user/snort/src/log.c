/* $Id: log.c,v 1.114 2003/04/09 21:12:24 chrisgreen Exp $ */
/*
...
/****************************************************************************
 *
 * Function: AlertFileName(char *, char *)
 *
 * Purpose: Determine alert filename
 *
 * Arguments: filearg => base filename
 *            filename => buffer to receive full filename
 *            length => length of filename buffer
 *
 * Returns: void function
 *
 ***************************************************************************/
static void AlertFileName(char *filearg, char *filename, int length)
{
    ...
    {
        if(!pv.daemon_flag)
            snprintf(filename, length, "%s/alert%s", pv.log_dir, suffix);
        else
            snprintf(filename, length, "%s/%s", pv.log_dir, 
...
FILE *OpenAlertFile(char *filearg)
{
    ...
    AlertFileName(filearg, filename, STD_BUF);
...
/****************************************************************************
 *
 * Function: RotateAlertFile(char *)
 *
 * Purpose: Rotate the log file when it is too big
 *
 * Arguments: filearg => the filename to open
 *            file => file handle of current log file
 *
 * Returns: file handle
 *
 ***************************************************************************/
#ifdef EMBED
#define DEFAULT_ALERT_FILE_LIMIT 16384
#else
#define DEFAULT_ALERT_FILE_LIMIT -1
#endif
FILE *RotateAlertFile(char *filearg, int limit, FILE *file)
{
    struct stat st;
    char filename[STD_BUF+1];
    char oldfilename[STD_BUF+1];

    if(limit == 0)
        limit = DEFAULT_ALERT_FILE_LIMIT;
    if(limit < 0)
        return file;
    if(fstat(fileno(file), &st) == -1 || st.st_size < limit)
        return file;

    AlertFileName(filearg, filename, STD_BUF);
    snprintf(oldfilename, STD_BUF, "%s.old", filename);
    rename(filename, oldfilename);
    fclose(file);

    return OpenAlertFile(filearg);
}
