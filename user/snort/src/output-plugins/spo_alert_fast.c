...
/* $Id: spo_alert_fast.c,v 1.18 2003/05/19 18:08:04 chrisgreen Exp $ */
...
SpoAlertFastData *ParseAlertFastArgs(char *args)
{
    ...
    data = (SpoAlertFastData *)calloc(1, sizeof(SpoAlertFastData));
    ...
    {
        if(strcasecmp(toks[i], "packet") == 0)
        {
            data->packet_flag = 1;
        }
        else if(strcasecmp(toks[i], "limit") ==0)
        {
            ++i;
	    if (i >= num_toks)
                FatalError("Missing alert_fast limit argument\n");
	    data->limit = strtol(toks[i], &end, 0);
	    if (*end || data->limit < LONG_MIN || data->limit == LONG_MAX)
                FatalError("Invalid alert_fast limit: %s\n", toks[i]);
        }
        else
        {
