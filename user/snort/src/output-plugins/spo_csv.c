...
/* $Id: spo_csv.c,v 1.19 2003/05/19 18:08:05 chrisgreen Exp $ */
...
#define DEFAULT_CSV "timestamp,msg,proto,src,srcport,dst,dstport,ethsrc,ethdst,ethlen,tcpflags,tcpseq,tcpack,tcpln,tcpwindow,ttl,tos,id,dgmlen,iplen,icmptype,icmpcode,icmpid,icmpseq"
...
AlertCSVData *AlertCSVParseArgs(char *args)
{
    ...
    data = (AlertCSVData *)malloc(sizeof(AlertCSVData));
...
{
    AlertCSVData *data = (AlertCSVData *)arg;
    RealAlertCSV(p, msg, data->file, data->args, data->numargs); 
    return;
}
