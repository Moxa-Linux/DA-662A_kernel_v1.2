...
*/
/* $Id: spo_alert_syslog.c,v 1.35 2003/05/19 18:08:04 chrisgreen Exp $ */
...
SyslogData *ParseSyslogArgs(char *args)
{
    ...
    config_toks = mSplit(args, ",", 2, &num_toks, '\\');
    switch( num_toks )
    {
    ...
    case 2:  /* config consists of host info, and facility/priority info */
        ...
        host_toks = mSplit(host_string, "=:", 3, &num_toks, 0);
        if( num_toks>0 && strcmp(host_toks[0], "host") != 0 )
        {
            FatalError("%s(%d) => Badly formed alert_syslog 'host' argument ('%s')\n", 
                       file_name, file_line, host_string);
        }
        /* check for empty strings */
        if( (num_toks>=1 && strlen(host_toks[0])==0) ||
            (num_toks>=2 && strlen(host_toks[1])==0) ||
            (num_toks>=3 && strlen(host_toks[2])==0)    )
        {
            FatalError("%s(%d) => Badly formed alert_syslog 'host' argument ('%s')\n", 
                       file_name, file_line, host_string);
        }
        ...
        case 3:  /* ie.  host=localhost:514 */
            ...
            {
                pv.syslog_server_port = DEFAULT_SYSLOG_PORT;  /* default */
                LogMessage("WARNING %s(%d) => alert_syslog port appears "
                           "to be non-numeric ('%s').  Defaulting to port %d!\n",
                           file_name, file_line, host_toks[2], DEFAULT_SYSLOG_PORT);
            }
            break;
    ...
    facility_toks = mSplit(facility_string, " |", 31, &num_toks, '\\');
