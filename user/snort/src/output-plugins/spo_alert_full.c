...
/* $Id: spo_alert_full.c,v 1.17 2003/05/19 18:08:04 chrisgreen Exp $ */
...
void AlertFull(Packet *p, char *msg, void *arg, Event *event)
{
    ...
    fflush(data->file);
    if (data->file != stdout)
        data->file = RotateAlertFile(data->filename, data->limit, data->file);
    return;
...
SpoAlertFullData *ParseAlertFullArgs(char *args)
{
    ...
    data = (SpoAlertFullData *)calloc(1, sizeof(SpoAlertFullData));
    ...
    toks = mSplit(args, " ", 31, &num_toks, 0);
    ...
    {
        data->filename = ProcessFileOption(toks[0]);
        data->file = OpenAlertFile(data->filename);
    }
    for(i = 1; i < num_toks; ++i)
    {
        if(strcasecmp(toks[i], "limit") ==0)
        {
            ++i;
	    if (i >= num_toks)
                FatalError("Missing alert_full limit argument\n");
	    data->limit = strtol(toks[i], &end, 0);
	    if (*end || data->limit < LONG_MIN || data->limit == LONG_MAX)
                FatalError("Invalid alert_full limit: %s\n", toks[i]);
        }
        else
        {
            FatalError("Unrecognized alert_full option: %s\n", toks[i]);
        }
    }
    return data;
}
