/* $Id: parser.c,v 1.85 2003/05/19 18:08:03 chrisgreen Exp $ */
/*
...
void ParseRule(FILE *rule_file, char *prule, int inclevel)
{
    ...
    */
    ...
    {
	FatalError("%s(%d): Illegal direction specifier: %s", toks[4]);
    }
...
void ParseRuleOptions(char *rule, int rule_type, int protocol)
{
    ...
    if(idx != NULL)
    {
        ...
        if(aux == NULL)
        {
            FatalError("%s(%d): %s Missing trailing ')' in rule: %s.\n",
        ...
        *aux = 0;
		...
		ONE_CHECK (one_logto, opts[0]);
            ...
            else if(!strcasecmp(opts[0], "activates"))
            {
		ONE_CHECK (one_activates, opts[0]);
                if(num_opts == 2)
                {
            ...
            else if(!strcasecmp(opts[0], "activated_by"))
            {
		ONE_CHECK (one_activated_by, opts[0]);
                if(num_opts == 2)
                {
            ...
            else if(!strcasecmp(opts[0], "count"))
            {
		ONE_CHECK (one_count, opts[0]);
                if(num_opts == 2)
                {
		...
		ONE_CHECK (one_rev, opts[0]);
            ...
            else if(!strcasecmp(opts[0], "reference"))
            {
                if(num_opts == 2)
                {
            ...
            else if(!strcasecmp(opts[0], "priority"))
            {
		ONE_CHECK (one_priority, opts[0]);
            ...
            else if(!strcasecmp(opts[0], "classtype"))
            {
		ONE_CHECK (one_classtype, opts[0]);
            ...
            else if(!strcasecmp(opts[0], "stateless"))
            {
		ONE_CHECK (one_stateless, opts[0]);
                otn_tmp->stateless = 1;
            }
            else if(!strcasecmp(opts[0], "threshold"))
            {
                ONE_CHECK (one_threshold, opts[0]);
                if(num_opts == 2)
                {
                    ParseThreshold(opts[1], &otn_tmp->threshold, 
                            &otn_tmp->window, &otn_tmp->threshold_type);
                }
                else
                {
                    FatalError("\n%s(%d) => No argument passed to "
            ...
            else
            {
                ...
                if(!found)
                {
                    ...
                    LogMessage("Warning: %s(%d) => Unknown keyword '%s' in "
...
#if 0 /* Relocated to parser/IpAddrSet.c */
/*
    ...
    switch(num_toks)
    {
        ...
        case 2:
            if(cidr)
            {
                ...
                {
                    FatalError("%s(%d) => Invalid CIDR block for IP addr %s\n", file_name, file_line, addr);
                }
            }
            else
            {
                ...
                {
                    FatalError("%s(%d) => Unable to parse rule netmask (%s)\n", file_name, file_line, toks[1]);
                }
            }
            break;
...
#endif /* Relocated to parser/IpAddrSet.c */
...
void ParseConfig(char *rule)
{
    ...
    else if(!strcasecmp(config, "disable_ttcp_alerts") || !strcasecmp(config, "disable_tcpopt_ttcp_alerts"))
    {
        /* dump the application layer */
