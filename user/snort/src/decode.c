/* $Id: decode.c,v 1.98 2003/05/13 15:55:32 chrisgreen Exp $ */
            ...
            if(cap_len < IEEE802_11_DATA_HDR_LEN + sizeof(EthLlc))
            {
                ...
                if((runMode == MODE_IDS) && pv.decoder_flags.decode_alerts)
                {
                    SetEvent(&event, GENERATOR_SNORT_DECODE,
                             DECODE_BAD_80211_ETHLLC, 1, DECODE_CLASS, 5, 0);
                    CallAlertFuncs(p, DECODE_BAD_80211_ETHLLC_STR, NULL, &event);
                    CallLogFuncs(p, DECODE_BAD_80211_ETHLLC_STR, NULL, &event);
                }
...
void DecodeVlan(u_int8_t * pkt, const u_int32_t len, Packet * p)
{
    ...
    if(len < sizeof(VlanTagHdr))
    {
        ...
        {
            SetEvent(&event, GENERATOR_SNORT_DECODE,
                     DECODE_BAD_VLAN, 1, DECODE_CLASS, 5, 0);
            CallAlertFuncs(p, DECODE_BAD_VLAN_STR, NULL, &event);
            CallLogFuncs(p, DECODE_BAD_VLAN_STR, NULL, &event);
        }
    ...
    if(ntohs(p->vh->vth_proto) <= ETHERNET_MAX_LEN_ENCAP)
    {
        if(len < sizeof(VlanTagHdr) + sizeof(EthLlc))
        {
            ...
            {
                SetEvent(&event, GENERATOR_SNORT_DECODE,
                         DECODE_BAD_VLAN_ETHLLC, 1, DECODE_CLASS, 5, 0);
                CallAlertFuncs(p, DECODE_BAD_VLAN_ETHLLC_STR, NULL, &event);
                CallLogFuncs(p, DECODE_BAD_VLAN_STR, NULL, &event);
            }
            return;            
        }
        ...
        if(p->ehllc->dsap == ETH_DSAP_IP && p->ehllc->ssap == ETH_SSAP_IP)
        {
            if(len < sizeof(VlanTagHdr) + sizeof(EthLlc) + sizeof(EthLlcOther))
            {
                ...
                {
                    SetEvent(&event, GENERATOR_SNORT_DECODE,
                             DECODE_BAD_VLAN_OTHER, 1, DECODE_CLASS, 5, 0);
                    CallAlertFuncs(p, DECODE_BAD_VLAN_OTHER_STR, NULL, &event);
                    CallLogFuncs(p, DECODE_BAD_VLAN_OTHER_STR, NULL, &event);
                }
...
void DecodeTRPkt(Packet * p, struct pcap_pkthdr * pkthdr, u_int8_t * pkt)
{
    ...
    if(cap_len < sizeof(Trh_hdr))
    {
        ...
        {
            SetEvent(&event, GENERATOR_SNORT_DECODE,
                     DECODE_BAD_TRH, 1, DECODE_CLASS, 5, 0);
            CallAlertFuncs(p, DECODE_BAD_TRH_STR, NULL, &event);
            CallLogFuncs(p, DECODE_BAD_TRH_STR, NULL, &event);
        }
    ...
    if(cap_len < (sizeof(Trh_hdr) + sizeof(Trh_llc)))
    {
        ...
        {
            SetEvent(&event, GENERATOR_SNORT_DECODE,
                     DECODE_BAD_TR_ETHLLC, 1, DECODE_CLASS, 5, 0);
            CallAlertFuncs(p, DECODE_BAD_TR_ETHLLC_STR, NULL, &event);
            CallLogFuncs(p, DECODE_BAD_TR_ETHLLC_STR, NULL, &event);
        }
        return;
    }
    ...
    if(p->trhllc->dsap != IPARP_SAP && p->trhllc->ssap != IPARP_SAP)
    {
        ...
        if(cap_len < (sizeof(Trh_hdr) + sizeof(Trh_llc) + sizeof(Trh_mr)))
        {
            ...
            {
                SetEvent(&event, GENERATOR_SNORT_DECODE,
                         DECODE_BAD_TRHMR, 1, DECODE_CLASS, 5, 0);
                CallAlertFuncs(p, DECODE_BAD_TRHMR_STR, NULL, &event);
                CallLogFuncs(p, DECODE_BAD_TRHMR_STR, NULL, &event);
            }
                      ...
                      sizeof(Trh_mr) + TRH_MR_LEN(p->trhmr)))
        {
            ...
            {
                SetEvent(&event, GENERATOR_SNORT_DECODE,
                         DECODE_BAD_TR_MR_LEN, 1, DECODE_CLASS, 5, 0);
                CallAlertFuncs(p, DECODE_BAD_TR_MR_LEN_STR, NULL, &event);
                CallLogFuncs(p, DECODE_BAD_TR_MR_LEN_STR, NULL, &event);                
            }
...
void DecodePPPoEPkt(Packet * p, struct pcap_pkthdr * pkthdr, u_int8_t * pkt)
{
    ...
    if(cap_len < PPPOE_HEADER_LEN)
    {
        ...
        if((runMode == MODE_IDS) && pv.decoder_flags.decode_alerts)
        {
            Event event;
            SetEvent(&event, GENERATOR_SNORT_DECODE, 
                    DECODE_BAD_PPPOE, 1, DECODE_CLASS, 5, 0);
            CallAlertFuncs(p, DECODE_BAD_PPPOE_STR, NULL, &event);
            CallLogFuncs(p, DECODE_BAD_PPPOE_STR, NULL, &event);
        }
 ...
 *
 * Purpose: Decode PPP traffic.  This really is intended to handle IPCP
 *
 * Arguments: p => pointer to decoded packet struct 
 *            user => Utility pointer, unused
 *            pkthdr => ptr to the packet header
...
void DecodeIP(u_int8_t * pkt, const u_int32_t len, Packet * p)
{
    ...
    if(IP_VER(p->iph) != 4)
    {
        ...
        {
            SetEvent(&event, GENERATOR_SNORT_DECODE, 
                    DECODE_NOT_IPV4_DGRAM, 1, DECODE_CLASS, 5, 0);
    ...
    if(hlen < IP_HEADER_LEN)
    {
        ...
        {
            SetEvent(&event, GENERATOR_SNORT_DECODE, 
                    DECODE_IPV4_INVALID_HEADER_LEN, 1, DECODE_CLASS, 5, 0);
    ...
    if(ip_len < hlen)
    {
        ...
        {
            SetEvent(&event, GENERATOR_SNORT_DECODE, 
                    DECODE_IPV4_DGRAM_LT_IPHDR, 1, DECODE_CLASS, 5, 0);
...
void DecodeTCP(u_int8_t * pkt, const u_int32_t len, Packet * p)
{
    ...
    if(len < 20)
    {
        ...
        {
            SetEvent(&event, GENERATOR_SNORT_DECODE, 
                    DECODE_TCP_DGRAM_LT_TCPHDR, 1, DECODE_CLASS, 5, 0);
...
void DecodeUDP(u_int8_t * pkt, const u_int32_t len, Packet * p)
{
    ...
    if(len < sizeof(UDPHdr))
    {
        ...
        {
            SetEvent(&event, GENERATOR_SNORT_DECODE, 
                    DECODE_UDP_DGRAM_LT_UDPHDR, 1, DECODE_CLASS, 5, 0);
    ...
    if(uhlen < UDP_HEADER_LEN)
    {
        ...
        {
            SetEvent(&event, GENERATOR_SNORT_DECODE, 
                    DECODE_UDP_DGRAM_INVALID_LENGTH, 1, DECODE_CLASS, 5, 0);
            CallAlertFuncs(p, DECODE_UDP_DGRAM_INVALID_LENGTH_STR, NULL, 
                    &event);
            CallLogFuncs(p, DECODE_UDP_DGRAM_INVALID_LENGTH_STR, NULL, &event);
            DisableDetect(p);
        }
        p->udph = NULL;
        pc.discards++;
    ...
    if(len < uhlen)
    {
        ...
        {
            SetEvent(&event, GENERATOR_SNORT_DECODE, 
                    DECODE_UDP_DGRAM_SHORT_PACKET, 1, DECODE_CLASS, 5, 0);
            CallAlertFuncs(p, DECODE_UDP_DGRAM_SHORT_PACKET_STR, NULL, &event);
            CallLogFuncs(p, DECODE_UDP_DGRAM_SHORT_PACKET_STR, NULL, &event);
            DisableDetect(p);                   
        }
    ...
    switch (p->icmph->type)
    {
        ...
        case ICMP_INFO_REPLY:
            if (len < 8)
            {
                ...
                {
                    SetEvent(&event, GENERATOR_SNORT_DECODE, 
                            DECODE_ICMP_DGRAM_LT_ICMPHDR, 1, DECODE_CLASS, 5, 0);
        ...
        case ICMP_TIMESTAMPREPLY:
            if (len < 20)
            {
                ...
                {
                    SetEvent(&event, GENERATOR_SNORT_DECODE, 
                            DECODE_ICMP_DGRAM_LT_TIMESTAMPHDR, 1, DECODE_CLASS, 5, 0);
        ...
        case ICMP_ADDRESSREPLY:
            if (len < 12)
            {
                ...
                {
                    SetEvent(&event, GENERATOR_SNORT_DECODE, 
                            DECODE_ICMP_DGRAM_LT_ADDRHDR, 1, DECODE_CLASS, 5, 0);
        ...
        case ICMP_DEST_UNREACH:
            {
                ...
                if(!DecodeIPOnly(pkt + 8, orig_p_caplen, p))
                {
                    ...
                    {
                        SetEvent(&event, GENERATOR_SNORT_DECODE, 
                                DECODE_IPV4_DGRAM_UNKNOWN, 1, DECODE_CLASS, 5, 0);
...
void DecodeARP(u_int8_t * pkt, u_int32_t len, Packet * p)
{
    ...
    if(len < sizeof(EtherARP))
    {
        ...
        {
            SetEvent(&event, GENERATOR_SNORT_DECODE, 
                    DECODE_ARP_TRUNCATED, 1, DECODE_CLASS, 5, 0);
...
void DecodeEapol(u_int8_t * pkt, u_int32_t len, Packet * p)
{
    ...
    if(len < sizeof(EtherEapol))
    {
        ...
        {
            SetEvent(&event, GENERATOR_SNORT_DECODE, 
                    DECODE_EAPOL_TRUNCATED, 1, DECODE_CLASS, 5, 0);
...
void DecodeEapolKey(u_int8_t * pkt, u_int32_t len, Packet * p)
{
    ...
    if(len < sizeof(EapolKey))
    {
        ...
        {
            SetEvent(&event, GENERATOR_SNORT_DECODE, 
                    DECODE_EAPKEY_TRUNCATED, 1, DECODE_CLASS, 5, 0);
...
void DecodeEAP(u_int8_t * pkt, const u_int32_t len, Packet * p)
{
    ...
    if(len < sizeof(EAPHdr))
    {
        ...
        {
            SetEvent(&event, GENERATOR_SNORT_DECODE, 
                    DECODE_EAP_TRUNCATED, 1, DECODE_CLASS, 5, 0);
...
void DecodeTCPOptions(u_int8_t *start, u_int32_t o_len, Packet *p)
{
    ...
    while((option_ptr < end_ptr) && (opt_count < TCP_OPTLENMAX) && (code >= 0) && !done)
    {
        ...
        if(code < 0)
        {
            ...
            {
                SetEvent(&event, GENERATOR_SNORT_DECODE,
                         DECODE_TCPOPT_BADLEN, 1,DECODE_CLASS, 3,0);
                CallAlertFuncs(p, DECODE_TCPOPT_BADLEN_STR, NULL, &event);
                CallLogFuncs(p, DECODE_TCPOPT_BADLEN_STR, NULL, &event);
                DisableDetect(p);
                return;
            }
            ...
            {
                SetEvent(&event, GENERATOR_SNORT_DECODE,
                         DECODE_TCPOPT_TRUNCATED, 1, DECODE_CLASS, 3, 0);
                CallAlertFuncs(p, DECODE_TCPOPT_TRUNCATED_STR, NULL, &event);
                CallLogFuncs(p, DECODE_TCPOPT_TRUNCATED_STR, NULL, &event);
                DisableDetect(p);
                return;
            }
            else
    ...
    if(experimental_option_found && pv.decoder_flags.tcpopt_experiment)
    {
        SetEvent(&event, GENERATOR_SNORT_DECODE,
                 DECODE_TCPOPT_EXPERIMENT, 1, DECODE_CLASS, 3, 0);
        CallAlertFuncs(p, DECODE_TCPOPT_EXPERIMENT_STR, NULL, &event);
        CallLogFuncs(p, DECODE_TCPOPT_EXPERIMENT_STR, NULL, &event);
        DisableDetect(p);
        return;
    }
    else if(obsolete_option_found && pv.decoder_flags.tcpopt_obsolete)
    {
        SetEvent(&event, GENERATOR_SNORT_DECODE,
                 DECODE_TCPOPT_OBSOLETE, 1, DECODE_CLASS, 3, 0);
        CallAlertFuncs(p, DECODE_TCPOPT_OBSOLETE_STR, NULL, &event);
        CallLogFuncs(p, DECODE_TCPOPT_OBSOLETE_STR, NULL, &event);
        DisableDetect(p);
        return;
    }
    else if(ttcp_found && pv.decoder_flags.tcpopt_ttcp)
    {
        SetEvent(&event, GENERATOR_SNORT_DECODE,
                 DECODE_TCPOPT_TTCP, 1, DECODE_CLASS, 3, 0);
        CallAlertFuncs(p, DECODE_TCPOPT_TTCP_STR, NULL, &event);
        CallLogFuncs(p, DECODE_TCPOPT_TTCP_STR, NULL, &event);
        DisableDetect(p);
        return;
    }
...
void DecodeIPOptions(u_int8_t *start, u_int32_t o_len, Packet *p)
{
    ...
    while((option_ptr < end_ptr) && (opt_count < 40) && (code >= 0))
    {
        ...
        if(code < 0)
        {
            ...
            {
                SetEvent(&event, GENERATOR_SNORT_DECODE,
                         DECODE_IPV4OPT_BADLEN, 1,DECODE_CLASS, 3,0);
                CallAlertFuncs(p, DECODE_IPV4OPT_BADLEN_STR, NULL, &event);
                CallLogFuncs(p, DECODE_IPV4OPT_BADLEN_STR, NULL, &event);
                DisableDetect(p);
                return;
            }
            ...
            {
                SetEvent(&event, GENERATOR_SNORT_DECODE,
                         DECODE_IPV4OPT_TRUNCATED, 1, DECODE_CLASS, 3, 0);
                CallAlertFuncs(p, DECODE_IPV4OPT_TRUNCATED_STR, NULL, &event);
                CallLogFuncs(p, DECODE_IPV4OPT_TRUNCATED_STR, NULL, &event);
                DisableDetect(p);
                return;
            }
            else
