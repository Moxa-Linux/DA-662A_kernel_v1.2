...
/* Aeluin Chen, 2014-01-13. add for updating a gps file - start */
static bool file_flag = false;
static char file_path[256];
/* Aeluin Chen, 2014-01-13. add for updating a gps file - end */
...
static void die(int sig)
{
    ...
    /* Aeluin Chen, 2014-01-13. do not operate the windows while using the file */
    if (!file_flag) {
...
/* This gets called once for each new compass sentence. */
static void update_gps_file(struct gps_data_t *gpsdata,
				 char *message, size_t len UNUSED)
{
    int newstate;
    char scr[128];
	...
	fprintf(fd, "GPS Type:%s\n", scr);
    }

    /* Fill in the gps number. */
    if (gpsdata->satellites_visible != 0)
	(void)snprintf(scr, sizeof(scr), "%d", gpsdata->satellites_visible);
    else
	(void)snprintf(scr, sizeof(scr), "%d", 0);
    fprintf(fd, "GPS Number:%s\n", scr);
...
{
    (void)fprintf(stderr,
		  "Usage: %s [-h] [-V] [-l {d|m|s}] [-f {file}] [server[:port:[device]]]\n\n"
		  ...
		  "		m = DD MM.mmmm'\n"
		  "		s = DD MM' SS.sss\"\n"
		  "  -f {file}  Write GPS information to file\n"
...
int main(int argc, char *argv[])
{
    ...
    while ((option = getopt(argc, argv, "hVl:smuD:f:")) != -1) {
	switch (option) {
#ifdef CLIENTDEBUG_ENABLE
	case 'D':
	    debug = atoi(optarg);
	...
	case 'f':
		...
		if (NULL != optarg)
			...
			strcpy(file_path, "/dev/shm/gps_info");
	    break;
	case 'h':
	default:
	    usage(argv[0]);
    ...
    /* Fire up curses. */
    /* Aeluin Chen, 2014-01-13. don't fire up the curses if using a file */
    if (!file_flag) {
    (void)initscr();
    (void)noecho();
    ...
    /* Aeluin Chen, 2014-01-13. don't setup the window if using a file */
    if (!file_flag) {
    windowsetup();
    }
    ...
    /* Aeluin Chen, 2014-01-13. add for updating a gps file - start */
    if (file_flag) {
    gps_set_raw_hook(&gpsdata, update_gps_file);
    } else
    /* Aeluin Chen, 2014-01-13. add for updating a gps file - end */
    {
