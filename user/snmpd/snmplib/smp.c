...
union snmp_mib_union	SnmpMib;

static AsnIdType snmpRetrieve(CIntfType item)
{
	AsnIdType	asnresult;

	if ( item < 1 || item > SNMP_MAX_ITEM )
		return 0;
	if ( item == SNMP_ENABLE_AUTHEN_TRAPS ) {
		asnresult = asnUnsl (asnClassUniversal, (AsnTagType)2, SnmpMib.snmp_mibs.EnableAuthenTraps);
		return asnresult;
	}
	asnresult = asnUnsl (asnClassApplication, (AsnTagType)1, SnmpMib.snmp_mibl[item-1]);
	return asnresult;
}

static AsnIdType snmpGet (MixCookieType cookie, MixNamePtrType name, MixLengthType namelen)
{
	CIntfType	item;

	if ( namelen != 2 || ((item=*name) < 1) || item > SNMP_MAX_ITEM || (*(name+1) != 0) )
		return 0;
	return snmpRetrieve(item);
}

static AsnIdType snmpNext (MixCookieType cookie, MixNamePtrType name, MixLengthPtrType namelenp)
{
	CIntfType	item;
	...
	if ( item < 1 || item >= SNMP_MAX_ITEM )
		return 0;
	if ( item == SNMP_IN_ASN_PARSE_ERRS )
		item = SNMP_IN_TOO_BIGS;
	else if ( item == SNMP_OUT_BAD_VALUES )
		item = SNMP_OUT_GEN_ERRS;
	else
		item++;
	*namelenp = 2;
	*name++ = item;
...
{
	// following add by Victor Yu. 08-02-2004
	memset(&SnmpMib, 0, sizeof(union snmp_mib_union));
	SnmpMib.snmp_mibs.EnableAuthenTraps = 2;	// disable
	(void) misExport ((MixNamePtrType) "\53\6\1\2\1\13",
		(MixLengthType)6, &snmpOps, (MixCookieType) 0);
	// above add by Victor Yu. 08-02-2004
}
...
static	SmpStatusType	smpSend (SmpIdType smp, ApsIdType aps, AsnIdType asn)
{
	...
	if (msg == (AsnIdType) 0) {
		SnmpMib.snmp_mibs.OutGenErrs++;	// add by Victor Yu. 08-02-2004
		return (errBad);
	}
	DEBUGASN (msg);
	DEBUG0 ("\n");
	...
	status = (*sp->smpRecSendFn) (sp->smpRecPeer,
		buffer, (CIntfType) n);
	msg = asnFree (msg);
	SnmpMib.snmp_mibs.OutPkts++;	// add by Victor Yu. 08-02-2004
	return (status);
}
...
static	AsnIdType	smpReply (AsnIdType asn0, SmpErrorType error, CUnsfType index, AsnIdType asn1)
{
	...
	SnmpMib.snmp_mibs.OutGetResponses++;	// add by Victor Yu. 08-02-2004
	return (result);
}
	...
	if (misCommunityToAccess (aps)) {
		...
		for (component = 1; (component <= sons) &&
			...
			if ((save = mixGet (mib, path, pathlen)) ==
				...
				SnmpMib.snmp_mibs.OutNoSuchNames++;	// add by Victor Yu. 08-02-2004
				DEBUG0 ("smpSetOp 1\n");
			}
		...
		if ((error == smpErrorNone) && (count >= smpMaxBindSize)) {
			...
			SnmpMib.snmp_mibs.OutTooBigs++;	// add by Victor Yu. 08-02-2004
		}
		while (count != 0) {
			count--;
			if (error != smpErrorNone) {
		...
		SnmpMib.snmp_mibs.InReadOnlys++;	// add by Victor Yu. 08-02-2004
	}
...
static	SmpStatusType	smpNextOp (SmpIdType smp, ApsIdType aps, AsnIdType asn)
{
	...
	rseq = asnSequence (asnClassUniversal, (AsnTagType) 0x10,
		asnTypeSequenceOf);
	if (rseq == (AsnIdType) 0) {
		SnmpMib.snmp_mibs.InGenErrs++;	// add by Victor Yu. 08-02-2004
		return (errBad);
	}
...
static	SmpStatusType	smpInputEvent (SmpIdType smp, CByteType x)
{
	...
	case 0x00:
		...
		// following add by Victor Yu. 08-02-2004
		if ( result == errOk ) {
			SnmpMib.snmp_mibs.InGetRequests++;	// add by Victor Yu. 08-02-2004
			SnmpMib.snmp_mibs.InTotalReqVars++;
		}
		// above add by Victor Yu. 08-02-2004
		break;
	...
	case 0x01:
		...
		// following add by Victor Yu. 08-02-2004
		if ( result == errOk ) {
			SnmpMib.snmp_mibs.InGetNexts++;	// add by Victor Yu. 08-02-2004
			SnmpMib.snmp_mibs.InTotalReqVars++;
		}
		// above add by Victor Yu. 08-02-2004
		break;
	...
	case 0x02:
		...
		// following add by Victor Yu. 08-02-2004
		if ( result == errOk ) {
			SnmpMib.snmp_mibs.InGetResponses++;	// add by Victor Yu. 08-02-2004
		}
		// above add by Victor Yu. 08-02-2004
		break;
	...
	case 0x03:
		...
		// following add by Victor Yu. 08-02-2004
		if ( result == errOk ) {
			SnmpMib.snmp_mibs.InSetRequests++;	// add by Victor Yu. 08-02-2004
		}
		// above add by Victor Yu. 08-02-2004
		break;
	...
	case 0x04:
		...
		// following add by Victor Yu. 08-02-2004
		if ( result == errOk ) {
			SnmpMib.snmp_mibs.InTraps++;	// add by Victor Yu. 08-02-2004
		}
		// above add by Victor Yu. 08-02-2004
		break;
...
SmpStatusType		smpRequest (SmpIdType smp, SmpRequestPtrType req)
{
	...
	if (smp == (SmpIdType) 0) {
		...
		case smpCommandGet:
			SnmpMib.snmp_mibs.OutGetRequests++;	// add by Victor Yu. 08-02-2004
			result = smpBuildReq ((AsnTagType) 0, req, list);
			break;

		case smpCommandNext:
			SnmpMib.snmp_mibs.OutGetNexts++;	// add by Victor Yu. 08-02-2004
			result = smpBuildReq ((AsnTagType) 1, req, list);
			break;

		case smpCommandSet:
			SnmpMib.snmp_mibs.OutSetRequests++;	// add by Victor Yu. 08-02-2004
		...
		case smpCommandTrap:
			SnmpMib.snmp_mibs.OutTraps++;	// add by Victor Yu. 08-02-2004
