...
asm_do_IRQ(unsigned int irq, struct pt_regs *regs)
{
	...
	if(irq == (14+32)) //irq number + 32 = fiq number (32 ~ 63 are fiq)
	{
		*(ptr_fiq + 2) = 0x00004000; // interrupt clear register
		moxaart_timer_interrupt_fiq(); // moxaart_timer_interrupt_fiq is located at moxaart_time.c
		return;
	}
#endif	

	/* XXX: do fixup for pci IRQ */
#ifdef CONFIG_ARCH_MOXAART
	irq = fixup_irq(irq);
	//desc = irq_desc + irq;
#endif
