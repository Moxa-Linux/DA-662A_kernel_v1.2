...
#if 0	// mask by Victor Yu. 11-08-2011
/* XXX */
void test_led() {
#define DATA_OUT (MOXAART_GPIO_VA_BASE)
#define DATA_IN (MOXAART_GPIO_VA_BASE + 4)
#define PIN_DIR (MOXAART_GPIO_VA_BASE + 8)
	putstr("led start");
	/* set to output */
	writel(readl(PIN_DIR) | 0x1111, PIN_DIR); 
	/* set to high */
	writel(readl(DATA_OUT) | 0X00, DATA_OUT);
	writel(readl(DATA_OUT) | 0X01, DATA_OUT);
	writel(readl(DATA_OUT) | 0X10, DATA_OUT);
	writel(readl(DATA_OUT) | 0X11, DATA_OUT);
	putstr("led end");
