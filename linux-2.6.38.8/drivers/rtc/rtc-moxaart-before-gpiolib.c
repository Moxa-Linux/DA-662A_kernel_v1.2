 ...
 * 	11-03-2011  Wade            Modify to new rtc driver type: refer to rtc-ps3.c 
 */
 ...
 * OTHER use 7/5/6
 */
#define GPIO_RTC_RESET		(1<<CONFIG_MOXAART_RTC_GPIO_RESET)
#define GPIO_RTC_SCLK		(1<<CONFIG_MOXAART_RTC_GPIO_SCLK)
#define GPIO_RTC_DATA		(1<<CONFIG_MOXAART_RTC_GPIO_DATA)
...
static u8 rtc_read(u8 cmd)
{
    ...
    moxaart_gpio_inout(GPIO_RTC_DATA, MCPU_GPIO_OUTPUT);
    moxaart_gpio_inout(GPIO_RTC_DATA, MCPU_GPIO_OUTPUT);
    moxaart_gpio_set(GPIO_RTC_RESET, MCPU_GPIO_HIGH);

	/* write command byte */
    for ( i=0; i<8; i++, cmd>>=1 ){
    	moxaart_gpio_set(GPIO_RTC_SCLK, MCPU_GPIO_LOW);
    	if ( cmd & 1 )
    		moxaart_gpio_set(GPIO_RTC_DATA, MCPU_GPIO_HIGH);
    	else
    		moxaart_gpio_set(GPIO_RTC_DATA, MCPU_GPIO_LOW);
    	moxaart_gpio_set(GPIO_RTC_SCLK, MCPU_GPIO_HIGH);
    }

    /* read data byte */
    moxaart_gpio_inout(GPIO_RTC_DATA, MCPU_GPIO_INPUT);

    for (i = 0, data =0; i < 8; i++) {
        moxaart_gpio_set(GPIO_RTC_SCLK, MCPU_GPIO_LOW);
        moxaart_gpio_set(GPIO_RTC_SCLK, MCPU_GPIO_HIGH);
        v = moxaart_gpio_get(GPIO_RTC_DATA);
        if (v) {
                data |= (1<<i);
        }
    } 
    ...
    moxaart_gpio_set(GPIO_RTC_SCLK, MCPU_GPIO_LOW);
    moxaart_gpio_set(GPIO_RTC_RESET, MCPU_GPIO_LOW);
...
static int __init moxaart_rtc_probe(struct platform_device *pdev)
{
	...
	moxaart_gpio_inout(GPIO_RTC_RESET|GPIO_RTC_SCLK|GPIO_RTC_DATA, MCPU_GPIO_OUTPUT);
