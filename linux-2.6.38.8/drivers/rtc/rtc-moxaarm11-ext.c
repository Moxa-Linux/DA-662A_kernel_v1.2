...
#define GPIO_RTC_RESET		11
#define GPIO_RTC_SCLK		12
#define GPIO_RTC_DATA		13
...
#define drv_err(fmt, args...) printk(KERN_ERR "MoxaARM11 external RTC: "fmt, ##args)
//#define drv_info(fmt, args...) printk(KERN_INFO "MoxaARM11 external RTC: "fmt, ##args)
#define drv_info(fmt, args...) printk("MoxaARM11 external RTC: "fmt, ##args)
...
static u8 rtc_read(u8 cmd)
{
    ...
    __gpio_set_value(GPIO_RTC_RESET, MCPU_GPIO_HIGH); 

	/* write command byte */
    for ( i=0; i<8; i++, cmd>>=1 ){
    	__gpio_set_value(GPIO_RTC_SCLK, MCPU_GPIO_LOW);
    	if ( cmd & 1 )
    		__gpio_set_value(GPIO_RTC_DATA, MCPU_GPIO_HIGH);
    	else
    		__gpio_set_value(GPIO_RTC_DATA, MCPU_GPIO_LOW);
    	__gpio_set_value(GPIO_RTC_SCLK, MCPU_GPIO_HIGH);
    }
    ...
    for (i = 0, data =0; i < 8; i++) {
        __gpio_set_value(GPIO_RTC_SCLK, MCPU_GPIO_LOW);
        __gpio_set_value(GPIO_RTC_SCLK, MCPU_GPIO_HIGH);
        v = __gpio_get_value(GPIO_RTC_DATA);
        if (v) {
                data |= (1<<i);
        }
    } 
    ...
    __gpio_set_value(GPIO_RTC_SCLK, MCPU_GPIO_LOW);
    __gpio_set_value(GPIO_RTC_RESET, MCPU_GPIO_LOW);
...
static int __init moxaart_rtc_probe(struct platform_device *pdev)
{
	...
	if ( tm.tm_sec == 0 && tm.tm_min == 0 && tm.tm_min == 0 && tm.tm_hour == 0 && tm.tm_year == 100 && tm.tm_mon == 0 && tm.tm_mday == 1 ) {
	...
	rtc = rtc_device_register("rtc-moxaarm11-ext", &pdev->dev, &moxaart_rtc_ops, THIS_MODULE);
	if (IS_ERR(rtc)) {
