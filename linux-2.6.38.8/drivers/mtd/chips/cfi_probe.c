				   ...
				   struct cfi_private *cfi)
{
...
#if 1	/* Jared, Set the write buffer size as 32 words for Micron's NOR flash. The write buffer size is supports 32, 64, 128, 256, 512 words. Default is 512 words. */
	cfi->cfiq->MaxBufWriteSize = ( le16_to_cpu(cfi->cfiq->MaxBufWriteSize) > 0x06 ) ? 0x06: le16_to_cpu(cfi->cfiq->MaxBufWriteSize);
#else
	 ...
	 */
#if 0 // Victor, J3D to J3F command issue
