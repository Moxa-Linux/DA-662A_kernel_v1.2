...
Retry_Sense:
		...
		usb_stor_show_sense(
			  ...
			  srb->sense_buffer[13]);
#endif
#if 0	// added by Victor Yu. 2015/6/1
if ( (srb->sense_buffer[2] & 0xf) == 0x06 && srb->sense_buffer[12] == 0x3a && srb->sense_buffer[13] == 0 ) {
...
int usb_stor_Bulk_max_lun(struct us_data *us)
{
	...
	US_DEBUGP("GetMaxLUN command result is %d, data is %d\n", 
		  result, us->iobuf[0]);
#if 0	// added by Victor Yu. 2015/5/14, because the storage is not ready, I set it to one,
	// after reading will be fail, the USB host will be reset this port, then the sotrage
	// will connect again good
	if ( us->iobuf[0] == 0 )
		us->iobuf[0] = 1;
#endif
