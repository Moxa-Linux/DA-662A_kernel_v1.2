...
#if defined(CONFIG_MACH_MOXA_UC8400) || defined(CONFIG_MACH_MOXA_UC8480) || (defined CONFIG_MACH_MOXA_DCU8620) // Jared, 08-30-2011
void     mountusb(char *action, char *usbdisk) {
	char    *argv[2], **envp, *buf, *scratch;
	int     i=0;

	if ( in_interrupt() )
		return;

	#if 0 // Jared, 2012-02-12, kernel migration issue
	if ( !current->fs->root )
		return;
	#endif

	if ( !(envp=(char **)kmalloc(10*sizeof(char *), GFP_KERNEL)) )
		return;
	...
	envp[i++] = scratch;
	scratch += sprintf(scratch, "DISK=%s", usbdisk ) + 1;
