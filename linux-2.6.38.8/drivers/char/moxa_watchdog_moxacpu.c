...
#define CONFIG_ARCH_MOXAART  // define for IA-24X/W3X1/W3X5 serials

#if defined (DA68X)
#include "x86_moxa_swtd.h"
#elif defined (CONFIG_CPU_IXP43X) || defined (ARCH_IXDP425) || defined (ARCH_IXDP422)
...
static void swtd_ack(unsigned long swtd_ack_time)
{
...
#elif defined (CONFIG_CPU_IXP43X) || defined (ARCH_IXDP425)
	...
	*IXP425_OSWK = 0;
#elif defined(CONFIG_ARCH_MOXAART)
	*(unsigned int *)(CPE_WATCHDOG_VA_BASE+4) = WATCHDOG_COUNTER(swtd_ack_time);
	*(unsigned int *)(CPE_WATCHDOG_VA_BASE+8) = 0x5ab9;
	/* Change to interrupt mode 0x05, reset mode is 0x03*/
	*(unsigned int *)(CPE_WATCHDOG_VA_BASE+12) = 0x05;
#elif defined(CONFIG_MACH_MOXA_IA261)||defined(CONFIG_MACH_MOXA_W406)
	// Not used
#endif
#endif
...
static void swtd_enable(void)
{
	...
	*IXP425_OSWK = 0;
#elif defined(CONFIG_ARCH_MOXAART)
	*(unsigned int *)(CPE_WATCHDOG_VA_BASE+4) = WATCHDOG_COUNTER(swtdtime+WATCHDOG_DEFER_TIME);
	*(unsigned int *)(CPE_WATCHDOG_VA_BASE+8) = 0x5ab9;
...
static void swtd_disable(void)
{
	...
	*IXP425_OSWK = 0;
#elif defined(CONFIG_ARCH_MOXAART)
	*(unsigned int *)(CPE_WATCHDOG_VA_BASE+12) = 0;
#elif defined(CONFIG_MACH_MOXA_IA261)||defined(CONFIG_MACH_MOXA_W406)
...
#if defined (DA68X) || defined (ARCH_IXDP425) || defined(CONFIG_ARCH_MOXAART)
DECLARE_WORK(rebootqueue, swtd_reboot, NULL);
#elif defined (CONFIG_CPU_IXP43X) || defined(CONFIG_MACH_MOXA_IA261) || defined(CONFIG_MACH_MOXA_W406)
...
static void swtd_poll(unsigned long ignore)
{
	...
	#endif
	bswtd_timeout = 1;
#else  // (CONFIG_CPU_IXP43X) || defined(CONFIG_MACH_MOXA_IA261) || defined(CONFIG_MACH_MOXA_W406) || defined(CONFIG_ARCH_MOXAART)
...
static void swtd_release_timer() {
...
#if 1 //For Tatung, 03-30-2009 should comment below
				if ( !bswtd_timeout  ) {
					swtd_release_timer();
				}
#endif
...
static irqreturn_t swtd_interrupt_handler(int irq, void *dev_id, struct pt_regs *regs)
{
	static int i = 3000;
	...
	sr =  *(unsigned char*)(CPE_FLASH_VA_BASE+0x55*2);

	if (sr & 0x04) {
		*( volatile u16 *)(CPE_FLASH_VA_BASE+0x55*2) = 0xd0;
		mdelay(10);
	} else if (sr & 0x40) {
		*( volatile u16 *)(CPE_FLASH_VA_BASE+0x55*2) = 0xd0;
		mdelay(10);
	...
	/* program suspend  */	
	*( volatile u16 *)(CPE_FLASH_VA_BASE+0x55*2) = 0xb0;
	...
	*( volatile u16 *)(CPE_FLASH_VA_BASE+0x55*2) = 0xff;
...
{	
	...
	*( volatile u16 *)(CPE_FLASH_VA_BASE+0x55*2) = 0xb0;//Suspend block erase
	*( volatile u16 *)(CPE_FLASH_VA_BASE+0x55*2) = 0xff;//Change to read array mode
...
static int __init moxaswtd_init(void) {
	...
	irq_res= request_irq(IRQ_WATCHDOG, swtd_interrupt_handler,
			SA_INTERRUPT,"swtd_irq",
			NULL);
	if (irq_res) {
	...
	notifier_chain_register(&panic_notifier_list, &swtd_panic_notifier);
#endif
...
static void __exit moxaswtd_exit(void) {
	...
	/* Free panic hander */
	notifier_chain_unregister(&panic_notifier_list, &swtd_panic_notifier);
#endif
