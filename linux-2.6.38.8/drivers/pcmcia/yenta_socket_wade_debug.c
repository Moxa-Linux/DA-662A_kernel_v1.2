...
static void yenta_set_power(struct yenta_socket *socket, socket_state_t *state)
{
	...
	if (!(cb_readl(socket, CB_SOCKET_STATE) & CB_CBCARD) &&
		...
		if (reg != old) {
            debugm("set I365_POWER=%x\n", reg);
			exca_writeb(socket, I365_POWER, reg);
        }
	} else {
		u32 reg = 0;	/* CB_SC_STPCLK? */
        debugm("state->Vcc=%d state->Vpp=%d\n", state->Vcc, state->Vpp);
		switch (state->Vcc) {
		case 33:
			reg = CB_SC_VCC_3V;
			break;
		...
		if (reg != cb_readl(socket, CB_SOCKET_CONTROL)) {
            debugm("set CB_SOCKET_CONTROL=%x\n", reg);
...
static int yenta_set_socket(struct pcmcia_socket *sock, socket_state_t *state)
{
    ...
    debugm("socket->io_irq=%d state->io_irq=%d\n", socket->io_irq, state->io_irq);
	...
	if (cb_readl(socket, CB_SOCKET_STATE) & CB_CBCARD) {
        ...
        debugm("set I365_CSCINT=%x\n", reg);
		exca_writeb(socket, I365_CSCINT, reg);
		exca_readb(socket, I365_CSC);
		if (sock->zoom_video)
			sock->zoom_video(sock, state->flags & SS_ZVCARD);
	}
    debugm("set CB_BRIDGE_CONTROL=%x\n", bridge);
	config_writew(socket, CB_BRIDGE_CONTROL, bridge);
	/* Socket event mask: get card insert/remove events.. */
	cb_writel(socket, CB_SOCKET_EVENT, -1);
    debugm("enable hot plug irq\n");
...
static unsigned int yenta_probe_irq(struct yenta_socket *socket, u32 isa_irq_mask)
{
    ...
    /* return a bit map of active interrupts */
...
static int yenta_probe_cb_irq(struct yenta_socket *socket)
{
    ...
    debugm("probe_status before request_irq=%d\n", socket->probe_status);
	if (request_irq(socket->cb_irq, yenta_probe_handler, IRQF_SHARED, "yenta", socket)) {
		dev_printk(KERN_WARNING, &socket->dev->dev,
			   "request_irq() in yenta_probe_cb_irq() failed!\n");
		return -1;
	}
    debugm("probe_status after request_irq=%d\n", socket->probe_status);
    ...
    /* cloase all event ?? */
	cb_writel(socket, CB_SOCKET_EVENT, -1);
    /* enable CSC interrupt whan card status change */
	cb_writel(socket, CB_SOCKET_MASK, CB_CSTSMASK);
    /* force enable a card status event */
	cb_writel(socket, CB_SOCKET_FORCE, CB_FCARDSTS);
...
static void yenta_config_init(struct yenta_socket *socket)
{
	...
	if ( dev->resource[0].flags & IORESOURCE_IO )
		config_writel(socket, PCI_BASE_ADDRESS_0, PHY_ADDRESS(region.start));
	else
		config_writel(socket, PCI_BASE_ADDRESS_0, MEM_PHY_ADDRESS(region.start));
#else	// CONFIG_ARCH_MOXAART
	config_writel(socket, PCI_BASE_ADDRESS_0, region.start);
#endif	// CONFIG_ARCH_MOXAART
...
#else
	config_writeb(socket, PCI_PRIMARY_BUS, dev->subordinate->primary & 0xff);
	config_writeb(socket, PCI_PRIMARY_BUS+1, dev->subordinate->secondary & 0xff);
	config_writeb(socket, PCI_PRIMARY_BUS+2, dev->subordinate->subordinate & 0xff);
	config_writeb(socket, PCI_PRIMARY_BUS+3, 0xb0);
#endif
	...
	config_writew(socket, CB_BRIDGE_CONTROL, bridge);
        ...
        {
        u32     _reg;
        _reg = config_readl(socket, 0x80);
        _reg |= 0x08000000;
        config_writel(socket, 0x80, _reg);
        }
#endif
}
...
static struct yenta_socket *socket;
...
static int __devinit yenta_probe(struct pci_dev *dev, const struct pci_device_id *id)
{
    ...
    debugm("call pcmcia register socket ret=%d\n", ret);
	if (ret == 0) {
        ...
        debugm("pcmcia_register_socket fail\n");
    }
...
int proc_read(char *buf, char **start, off_t offset,
                   int len, int *eof, void *data)
{
    ...
    len += sprintf(buf + len, "socket mask register=%x\n", cb_readl(socket, CB_SOCKET_MASK)); 
    len += sprintf(buf + len, "socket state register=%x\n", cb_readl(socket, CB_SOCKET_STATE)); 
    len += sprintf(buf + len, "I365_CSC=%x\n", exca_readb(socket, I365_CSC)); 
    len += sprintf(buf + len, "I365_CSCINT=%x\n", exca_readb(socket, I365_CSCINT)); 
    yenta_interrogate(socket);
    len += sprintf(buf + len, "socket state register=%x\n", cb_readl(socket, CB_SOCKET_STATE)); 
    //cb_writel(socket, CB_SOCKET_FORCE, CB_FCDETECT1); 
    //cb_writel(socket, CB_SOCKET_FORCE, CB_FCDETECT2); 
...
{
    debugm("---------------------v1.2--------------------------\n");
	create_proc_read_entry("wade_debug", 0, NULL, proc_read, NULL);
...
{
	remove_proc_entry("wade_debug", NULL); 
