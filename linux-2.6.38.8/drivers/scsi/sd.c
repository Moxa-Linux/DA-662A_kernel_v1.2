...
sd_spinup_disk(struct scsi_disk *sdkp)
{
	...
	do {
		...
		do {
			 ...
			 */
			if (media_not_present(sdkp, &sshdr)) {
#if 0	// added by Victor Yu. 2015/6/2
				#include <scsi/sg.h>
				int	val;
				val = SCSI_TRY_RESET_TARGET;
				scsi_nonblockable_ioctl(sdkp->device, SG_SCSI_RESET, &val, 1);
...
static int sd_revalidate_disk(struct gendisk *disk)
{
	...
	if (sdkp->media_present) {
		...
		int	val;
		val = SCSI_TRY_RESET_TARGET;
		scsi_nonblockable_ioctl(sdkp->device, SG_SCSI_RESET, &val, 0);
printk("%s,%s,%d\n", __FILE__, __FUNCTION__, __LINE__);
	}
#endif
...
extern void     mountusb(char *action, char *usbdisk);
#endif
/*
...
#if  defined(CONFIG_MACH_MOXA_UC8400) || defined(CONFIG_MACH_MOXA_UC8480) || defined(CONFIG_MACH_MOXA_DCU8620)
	// Jared, 08-30-2011, we disable the properitary usb automount and move to use mdev
	//mountusb("add", gd->disk_name);
#endif
...
static int sd_probe(struct device *dev)
{
	...
	if (error)
		goto out_free_index;
#if 1	// added by Victor Yu. 2015/5/13, I want to let the CF card is always mounted to /mnt/cf
//printk("%s,%s,%d:gd->disk_name=%s\n", __FILE__, __FUNCTION__, __LINE__, gd->disk_name);
{
static int	cfindex=0;
struct us_data {
...
//printk("%s,%s,%d:vendor=%x,product=%x\n", __FILE__, __FUNCTION__, __LINE__, us->pusb_dev->descriptor.idVendor, us->pusb_dev->descriptor.idProduct);
if ( us->pusb_dev->descriptor.idVendor == 0x05e3 && us->pusb_dev->descriptor.idProduct == 0x0716 ) {
	sprintf(gd->disk_name, "cf%c", cfindex+'a');
	cfindex++;
}
}
#endif
...
#if defined(CONFIG_MACH_MOXA_UC8400) || defined(CONFIG_MACH_MOXA_UC8480) || defined(CONFIG_MACH_MOXA_DCU8620)
	// Jared, 08-30-2011, we disable the properitary usb automount and move to use mdev
	//mountusb("remove", sdkp->disk->disk_name);
#endif
