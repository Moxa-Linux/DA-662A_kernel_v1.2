...
static __devinit struct net_device * rtl8139_init_board (struct pci_dev *pdev)
{
	...
	u32 version;
	#if 1 /* Jared, 2013/01/21, Because the latest DCU-8620 remove the PHY of fiber and lead to the ATSAGENT test fail.
		 Victor suggest to keep the original Ethernet name, so we need to naming the device of 8139too from eth2 */
	...
	#if 1 /* Jared, 2013/01/21, Because the latest DCU-8620 remove the PHY of fiber and lead to the ATSAGENT test fail.
		 Victor suggest to keep the original Ethernet name, so we need to naming the device of 8139too from eth2 */
		sprintf(dev->name, "eth%d", name_count++);
	#endif
...
// Jared: 10-28-2008. 
// Because UC8400 eth2/3 interfaces scan orders are reserved, we need to swap
// the interface name for register_netdev();
#if defined(CONFIG_MACH_MOXA_UC8400) 
	#include <linux/string.h>

	if ( strncmp(pci_name(pdev), "0000:00:01.0", strlen(pci_name(pdev))) == 0 )
