...
static int __devinit eth_init_one(struct platform_device *pdev)
{
...
#if 1 /*Jared, 07-25-2011, read the macaddr from redboot config*/
	#include	<mach/redboot_config.h>

	if( NPE_ID(port->id) == 0 )
		config_get("mac0_addr", plat->hwaddr);
	else
		config_get("mac1_addr", plat->hwaddr);
#endif
	...
	if ( port->id == IXP4XX_ETH_NPEA ) {
		__raw_writel(DEFAULT_CORE_CNTRL | CORE_RESET, &port->regs->core_control);
		mdelay(50);
	}
