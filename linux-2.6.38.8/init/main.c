...
#define DEBUG_LED		(DEBUG_LED_0|DEBUG_LED_1|DEBUG_LED_2|DEBUG_LED_3)
	...
	/* XXX: must after setup arch */
	/* 
	moxaart_gpio_inout_phy(DEBUG_LED, MCPU_GPIO_OUTPUT);
	moxaart_gpio_set_phy(DEBUG_LED_0, MCPU_GPIO_LOW);
	moxaart_gpio_set_phy(DEBUG_LED_1, MCPU_GPIO_LOW);
	moxaart_gpio_set_phy(DEBUG_LED_2, MCPU_GPIO_LOW);
	moxaart_gpio_set_phy(DEBUG_LED_3, MCPU_GPIO_LOW);
	*/


	char * command_line;
	extern const struct kernel_param __start___param[], __stop___param[];
#if 0  // add by Victor Yu. for debug using 07-27-2007, Fraddy tell me this issue
 ...
 */
	...
	setup_per_cpu_areas();
#if 0	// add by Victor Yu. for debug using 07-27-2007, Fraddy tell me this issue
	//printk("start_kernel: lock TLB\n");
	rd = 0xF9400003;	// garbage address for lock
	for ( tlb=0; tlb<2; tlb++ ) {
		for (way=0; way<4; way++ ) {
			for (entry=0; entry<16/*32*/; entry++ ) {
				/*
				if ( ((tlb == 0) && (entry == 0)) ||
				     ((tlb == 0) && (entry == 1)) ) {
				     */
