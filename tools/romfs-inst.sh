...
$0: [options] [src] dst
    ...
    -d directory: dst is a directory to be created
...
file_copy()
{
	...
	then
		...
		(
			...
			V=
			[ "$v" ] && V=v
			find . -print | grep -E -v '/CVS' | cpio -p${V}dum${link} ${ROMFSDIR}${dst}
		)
	else
		if [ -d "${dst}" ]
		then
			rm -f ${ROMFSDIR}${dst}${src}
		else
			rm -f ${ROMFSDIR}${dst}
		fi
		[ "$v" ] && echo "cp ${src} ${ROMFSDIR}${dst}"
		cp -ax ${src} ${ROMFSDIR}${dst}
...
dir_make()
{
	...
	[ "$v" ] && echo "mkdir -p ${ROMFSDIR}${dst}"
	mkdir -p ${ROMFSDIR}${dst}
	setperm ${ROMFSDIR}${dst}
}
...
while getopts 've:o:A:p:a:l:s:d:n' opt "$@"
do
	case "$opt" in
	v) v="1";                           ;;
	o) option="$OPTARG";                ;;
